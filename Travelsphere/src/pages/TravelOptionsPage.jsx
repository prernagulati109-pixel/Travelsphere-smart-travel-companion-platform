    ...
    <div className="explore-page">
            ...
            {paymentStep === 'summary' ? (
              <>
                <div style={{ textAlign: 'center', marginBottom: '30px' }}>
                  <div style={{ width: '60px', height: '60px', background: '#ecfdf5', borderRadius: '50%', display: 'grid', placeItems: 'center', margin: '0 auto 15px' }}>
                    <span style={{ fontSize: '30px' }}>{modeIcon}</span>
                  </div>
                  <h2 style={{ margin: 0 }}>Confirm Your {activeMode}</h2>
                  <p style={{ color: '#64748b', marginTop: '8px' }}>Review your {bookedItem.name} booking</p>
                </div>
                ...
                <div style={{ background: '#f8fafc', borderRadius: '16px', padding: '20px', marginBottom: '24px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                    <span>Operator</span>
                    <strong>{bookedItem.name}</strong>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                    <span>Type/Class</span>
                    <strong>{bookedItem.type}</strong>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #e2e8f0', paddingTop: '10px', marginTop: '10px' }}>
                    <span>Total Amount</span>
                    <strong style={{ fontSize: '1.2rem', color: '#1e293b' }}>₹{bookedItem.price.toLocaleString()}</strong>
                  </div>
                </div>
              ...
              <>
                <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                  ...
                  ><ArrowRight size={16} style={{ transform: 'rotate(180deg)' }} /> Back</button>
                ...
                <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', background: '#f1f5f9', padding: '4px', borderRadius: '10px' }}>
                  <button 
                    ...
                    style={{ flex: 1, padding: '8px', border: 'none', borderRadius: '7px', background: paymentMethod === 'card' ? 'white' : 'transparent', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
                  ><CreditCard size={16} /> Card</button>
                  <button 
                    onClick={() => setPaymentMethod('scanner')}
                    style={{ flex: 1, padding: '8px', border: 'none', borderRadius: '7px', background: paymentMethod === 'scanner' ? 'white' : 'transparent', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
                  ><Smartphone size={16} /> Scanner</button>
                </div>

                {paymentMethod === 'card' ? (
                  <div style={{ display: 'grid', gap: '12px', textAlign: 'left' }}>
                    <div>
                      <label style={{ fontSize: '0.8rem', color: '#64748b', display: 'block', marginBottom: '4px' }}>Cardholder Name</label>
                      <input type="text" placeholder="John Doe" style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #e2e8f0' }} />
                    </div>
                    <div>
                  ...
                  <div style={{ textAlign: 'center', padding: '10px' }}>
                    <div style={{ width: '150px', height: '150px', margin: '0 auto', border: '2px solid #f1f5f9', borderRadius: '12px', padding: '8px', position: 'relative' }}>
                      <img src="/upi_qr_payment_mockup_1778255378343.png" alt="QR" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                      <div style={{ position: 'absolute', bottom: '-10px', left: '50%', transform: 'translateX(-50%)', background: '#2563eb', color: 'white', padding: '2px 10px', borderRadius: '10px', fontSize: '0.7rem', fontWeight: 'bold' }}>Scan & Pay</div>
                    </div>
                    <p style={{ fontSize: '0.8rem', color: '#64748b', marginTop: '15px' }}>Scan with any UPI app (GPay, PhonePe, Paytm)</p>
                  </div>
                )}
                ...
                >
                  {bookingLoading ? (
                    <><RefreshCcw size={18} className="spin" /> Verifying...</>
                  ) : `Pay ₹${bookedItem.price.toLocaleString()}`}
                </button>
              </>
            )}
          </div>
      ...
      {isBooked && (
        <div style={{
          ...
          <div>
            <h4 style={{ margin: 0 }}>Ticket Booked!</h4>
            <p style={{ margin: '4px 0 0', fontSize: '0.85rem', color: '#64748b' }}>Your {activeMode} ticket has been confirmed.</p>
          </div>
          <button onClick={() => setIsBooked(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#cbd5e1' }}><X size={16} /></button>
        </div>
      )}
